...
// Entity : RDBMS의 테이블과 매핑되는 ORM(JPA)의 클래스 (좁은 의미)
// - 웹 애플리케이션 서버에서 데이터베이스 서버와 데이터 전송 및 관리를 위한 객체 (넓은 의미)
...
public class SampleTable1Entity {
  ...
  // - name 속성 : 매핑할 컬럼명을 작성해줌
  // - nullable 속성 : null 포함 여부 확인
  // - unique 속성 : unique 제약 여부 확인
  // - length 속성 : 컬럼의 길이(일반적으로 문자열을 다룸)
...
// @ManyToOne, @OneToMany, @OneToOne...등과 같은 관계를 위한 어노테이션도 존재함
