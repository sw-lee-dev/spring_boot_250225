...
public class JwtProvider {
  ...
  // 해결책
  // 1. application.properties / application.yaml에 등록 (가장 흔히 사용, 비용 X)
  // - application.properties / application.yaml에 비밀키를 작성
  // - @Value() 어노테이션을 사용하여 값을 가져옴
  // - 주의사항 : application.properties / appliction.yaml을 .gitignore에 등록해야함
  @Value("${jwt.key}")
  private String secretKey;

  // 2. 시스템의 환경변수로 등록하여 사용 (1번보다 조금 더 안전, 내부 시스템에 한정적인 불편성)
  // - OS 자체의 시스템 환경변수에 비밀키를 등록
  // - Spring에서 시스템 환경변수를 읽어서 사용

  // 3. 외부 데이터 관리 도구를 사용 (가장 안전, 비용 높음)
  // - 자체 서버가 아닌 타 서버에 등록된 Vault도구를 사용하여 비밀키 관리
  // - OS 부팅시에 Vault 서버에서 비밀키를 가져와 사용
  // - OS 부팅시 마다 새로운 비밀키를 자동으로 부여해줌
