...
@Configurable
// @Configuration :
// - '메서드'가 호출 시 Spring Bean으로 등록될 수 있도록 하는 어노테이션
@Configuration
// @EnableWebSecurity : 
...
public class WebSecurityConfig {
  ...
  // @Bean : 
  // @Component를 사용하지 못하거나 사용하고 싶지 않을 때 Spring Bean으로 등록할 수 있도록 하는 어노테이션
  // WebSecurityConfig 클래스의 인스턴스 생성을 Spring에게 넘기지 않고 configure 메서드 호출만 Spring 에게 넘기는 것
  @Bean
  protected SecurityFilterChain configure(HttpSecurity security) throws Exception {

    // Class::method
    // - 메서드 참조, 특정 클래스의 메서드를 참조할 때 사용 (Java는 모든 함수가 클래스에 있는 메서드이기 때문)
    // - 매개변수로 메서드를 전달하고자 할 때 자주 사용 (JS, TS의 콜백 형태와 동일)
    security
      // Basic 인증 방식에 대한 설정
      // -> Basic 인증 방식 미사용으로 지정 (Bearer 인증 방식을 사용할 것이기 때문)
      ...
      // CSRF 취약점에 대한 대비 설정
      // -> CSRF 취약점에 대한 대비를 하지 않겠다고 지정 (세션을 유지하지 않겠다고 지정했기 때문에 탈취당할 세션이 없음 >> 보안이 올라가면 성능이 떨어지기 때문에 보안점 설정 X)
      ...
      // - 1.모든 클라이언트가 접근할 수 있도록 허용
      // - 2.인증된 모든 클라이언트가 접근할 수 있도록 허용
      // - 3.인증된 클라이언트 중 특정 권한을 가진 클라이언트만 접근할 수 있도록 허용
      .authorizeHttpRequests(request -> request
        ...
        // hasRole(권한) : 특정 권한을 가진 클라이언트만 접근할 수 있도록 지정 (매개변수로 전달하는 권한명은 "ROLE_"를 제거한 실제 권한명)
      ...
      // OAuth2 인증 처리 25.03.11 oauth 작업
      ...
      // 필터 등록
      // addFilterBefore(추가할 필터 인스턴스, 특정 필터 클래스) : 특정 필터 이전에 지정한 필터를 추가
...
// 인증 절차가 필요 >> 인증된 사용자로 JWT 발행 >> 보내려는 요청 Header에 Authorization 필드 값으로 Bearer JWT를 포함하여 전송
// .anyRequest().authenticated() >> 인가가 허용되지 않은 요청에 대해서는 인증이 필요함